...
#' 
#' Lin YV, Château P-A, Nozawa Y, Wei C-L, Wunderlich RF, Denis V (submitted) Environmental drivers of coastal benthic communities in a complex environmental setting. DOI: XXX
#' 
...
#' 
#' The outcomes from the k-means clustering, conditional random forest models, and the variable importance from models are slightly different every time when running, so we pre-save the outcomes of the current study and used it here to make sure the results showing here are consistent with the study.
#' 
## ----eval = TRUE, echo=T, message=F, warning=F--------------------------------
#k.means <- NbClust(benthic.fct.hell, diss = NULL, method = "kmeans" ,min.nc=2, max.nc=15, index = "all") # k-means - Appendix C

k.means <- readRDS("Data/k.means.RData") # read the outcome in the current study  
...
c.rf.near.tran <- readRDS('Data/c.rf.near.tran.BC.RData')
...
c.importance <- readRDS('Data/c.importance.test.RData')
...
c.rf.near.tran.BC1 <- readRDS('Data/c.rf.near.tran.BC1.RData')

#' 
#' **Variable importance**
## ----eval = TRUE, echo=T, message=F, warning=F--------------------------------
#c.importance.BC1 <- varimp(c.rf.near.tran.BC1, conditional = T)
c.importance.BC1 <- readRDS('Data/c.importance.BC1.RData')
...
## ----eval = TRUE, echo=T, message=F, warning=F--------------------------------
load("Data/c.rf.BC1.partial.RData")
...
#                               controls = cforest_unbiased(mtry = 5, ntree = 2000)) # build the random forest model
c.rf.near.tran.BC2 <- readRDS('Data/c.rf.near.tran.BC2.RData')
...
c.importance.BC2 <- readRDS('Data/c.importance.BC2.RData')
...
#' **Partial dependence plots for each environmental factor - Appendix J**
## ----eval = TRUE, echo=T, message=F, warning=F--------------------------------
load("Data/c.rf.BC2.partial.RData")
par(mfrow=c(6,4), mar = c(4,2, 1, 2))
...
#                              controls = cforest_unbiased(mtry = 5, ntree = 2000)) # build the random forest model
c.rf.near.tran.BC3 <- readRDS('Data/c.rf.near.tran.BC3.RData')

#' 
#' **Variable importance**
## ----eval = TRUE, echo=T, message=F, warning=F--------------------------------
#c.importance.BC3 <- varimp(c.rf.near.tran.BC3, conditional = T)
c.importance.BC3 <- readRDS('Data/c.importance.BC3.RData')
...
#' **Partial dependence plots for each environmental factor - Appendix J**
## ----eval = TRUE, echo=T, message=F, warning=F--------------------------------
load("Data/c.rf.BC3.partial.RData")
par(mfrow=c(6,4), mar = c(4,2, 1, 2))
...
# don't run since it takes for a while
#c.rf.BC4.mintree <- cforest.best.mtree(c.df.near.tran.BC2,5,20,3500) # MSEs become stable when mtree > 1000, so setup mtree = 2000
...
#                              controls = cforest_unbiased(mtry = 5, ntree = 2000)) # build the random forest model
c.rf.near.tran.BC4 <- readRDS('Data/c.rf.near.tran.BC4.RData')

#' 
#' **Variable importance**
## ----eval = TRUE, echo=T, message=F, warning=F--------------------------------
#c.importance.BC4 <- varimp(c.rf.near.tran.BC4, conditional = T)
c.importance.BC4 <- readRDS('Data/c.importance.BC4.RData')
...
#' **Partial dependence plots for each environmental factor - Appendix J**
## ----eval = TRUE, echo=T, message=F, warning=F--------------------------------
load("Data/c.rf.BC4.partial.RData")
par(mfrow=c(6,4), mar = c(4,2, 1, 2))
...
# don't run since it takes for a while
#c.rf.BC5.mintree <- cforest.best.mtree(c.df.near.tran.BC5,5,20,3500)  # MSEs become stable when mtree > 1000, so setup mtree = 2000
...
c.rf.near.tran.BC5 <- readRDS('Data/c.rf.near.tran.BC5.RData')

#' 
#' **Variable importance**
## ----eval = TRUE, echo=T, message=F, warning=F--------------------------------
#c.importance.BC5 <- varimp(c.rf.near.tran.BC5, conditional = T)
c.importance.BC5 <- readRDS('Data/c.importance.BC5.RData')
...
#' **Partial dependence plots for each environmental factor - Appendix J**
## ----eval = TRUE, echo=T, message=F, warning=F--------------------------------
load("Data/c.rf.BC5.partial.RData")
par(mfrow=c(6,4), mar = c(4,2, 1, 2))
