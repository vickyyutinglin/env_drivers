...
benthic.fct <- read.csv('Linetal_dataset_Benthic.csv',header = T) # import the benthic matrix
...
env.info <- read.csv('Linetal_dataset_Env.csv',header = T )# import the environmental driver matrix
coord.df <- read.csv('Linetal_dataset_Cor.csv' , head = T , sep = ",")
...
k.means <- NbClust(benthic.fct.hell, diss = NULL, method = "kmeans" ,min.nc=2, max.nc=15, index = "all") # k-means
k.means # Appendix C
...
ind.df <- round(ind$sign,3) # Appendix F
...
# use GAM to fit latitudes of transects to nMDS axes
nmds.sf <- ordisurf(nmds.fct, env.info$Latitude, plot=T) # fit latitudes of transects to nMDS axes
...
hist(env.info$Anthropogenic_land_use)
hist(env.info$Forest_land_use)
hist(env.info$Population_density)
hist(env.info$Tourist_visitors)
hist(env.info$Unstable_substrate_cover)
...
########################################
### best hyperparameters to build models

# the ideal number of splits (mtry) is suggested as the square root of the number of explained factors
# accordingly, the ideal number of splits for this study is the square root of 21, which is between 4 to 5.
# therefore, we decide mtry = 5
...
env.tran.all <- env.info[,c(6:26)]
BC <- as.factor(stability.clu$BC)
c.forest.benthic.env <- data.frame(env.tran.all, BC)
...
barplot(main = 'General CRF', sort(c.importance),horiz = T,las=1, xlim = c(0,0.02),cex.names=1.5,cex.main=3, cex.lab=1.5,
        col='white', alpha = 0.5, xlab="Mean decrease in accuracy")
abline(v = imp.mean ,col="red4",lty=2) # Appendix I
 
# model accuracy - Appendix H
pred.c.rf.tran.oob <- predict(c.rf.near.tran, OOB = TRUE) # calculate the predict values using general CRF 
pred.sum <- sum(table(pred.c.rf.tran.oob, c.forest.benthic.env$BC)) # check if the prediction fits the observation
pred.err.OOB <- (8+7+3+13+22+5+15+22+1+4+1+8)/ pred.sum # calculate out-of-bag error of general CRF
pred.err.OOB

### individual conditional random forests (CRFs)
### Community 1
c.df.near.tran.BC1 <- data.frame(env.tran.all,BC1)
c.rf.BC1.mintree <- cforest.best.mtree(c.df.near.tran.BC1,5,50,3000)
...
barplot(main = 'Community 1', sort(c.importance.BC1),horiz = T,las=1, xlim = c(0,0.025),cex.names=1.5,
        cex.main=3,cex.lab=1.5,col='#FF66FF', alpha = 0.5, xlab="Mean decrease in accuracy")
...
resid.BC1 <- pred.c.rf.tran.oob.BC1- c.df.near.tran.BC1$BC1
mse.BC1 <- mean(resid.BC1^2)
plot(c.df.near.tran.BC1$BC1,resid.BC1, pch=18,ylab="residuals (predicted-observed)",   xlab="observed",col="blue3") # residual plot
...
par(mfrow=c(6,4), mar = c(5.1,11, 4.1, 2.1),cex.lab=2,cex.axis=2)
...
c.rf.BC2.mintree <- cforest.best.mtree(c.df.near.tran.BC2,5,20,3500)
...
par(mar = c(5.1,18.5, 4.1, 2.1))
barplot(main = 'Community 2', sort(c.importance.BC2),horiz = T,las=1, xlim = c(0,0.005),cex.names=1.5,cex.main=3,
        col='#99CCFF', alpha = 0.5, xlab="Mean decrease in accuracy",cex.lab=1.5)
abline(v = imp.mean.BC2,col="red4",lty=2)

# model accuracy - Appendix H
pred.c.rf.tran.oob.BC2 <- predict(c.rf.near.tran.BC2, OOB = TRUE, c.df.near.tran.BC2) #get predicted (fitted) values
resid.BC2 <- pred.c.rf.tran.oob.BC2- c.df.near.tran.BC2$BC2
mse.BC2 <- mean(resid.BC2^2)
plot(c.df.near.tran.BC2$BC2,c.rf.resid.BC2, pch=18,ylab="residuals (predicted-observed)", xlab="observed",col="blue3") # residual plot
abline(h=0,col="red4",lty=2)
...
c.rf.BC3.mintree <- cforest.best.mtree(c.df.near.tran.BC3,5,20,3500)
...
par(mar = c(5.1,18.5, 4.1, 2.1))
barplot(main = 'Community 3', sort(c.importance.BC3),horiz = T,las=1, xlim = c(0,0.006),cex.names=1.5,cex.main=3,
...
plot(c.df.near.tran.BC3$BC3,resid.BC3, pch=18,ylab="residuals (predicted-observed)", xlab="observed",col="blue3") # residual plot
abline(h=0,col="red4",lty=2)
...
c.rf.BC4.mintree <- cforest.best.mtree(c.df.near.tran.BC2,5,20,3500)
...
par(mar = c(5.1,18.5, 4.1, 2.1))
barplot(main = 'Community 4', sort(c.importance.BC4),horiz = T,las=1, xlim = c(0,0.005),cex.names=1.5,cex.main=3,
...
plot(c.df.near.tran.BC4$BC4,resid.BC4, pch=18,ylab="residuals (predicted-observed)", xlab="observed",col="blue3") # residual plot
abline(h=0,col="red4",lty=2)
...
c.rf.BC5.mintree <- cforest.best.mtree(c.df.near.tran.BC5,5,20,3500)
...
par(mar = c(5.1,18.5, 4.1, 2.1))
barplot(main = 'Community 5', sort(c.importance.BC5),horiz = T,las=1, xlim = c(0,0.02),cex.names=1.5,cex.main=3,
...
plot(c.df.near.tran.BC5$BC5,resid.BC5, pch=18,ylab="residuals (predicted-observed)", xlab="observed",col="blue3") # residual plot
abline(h=0,col="red4",lty=2)
...
### Figure 1a - map
taiwan <- readOGR('Linetal_dataset_TW.shp') # import map data
taiwan_map <- fortify(taiwan)  # change .shp file into dataframe
...
ggplot() +
  ...
  geom_text_repel(data= coord.df, aes(x = longitude, y = latitude,label=ID),max.overlaps=Inf,
...
ggplot(coord.df, aes(x = 1, y= site)) +   # plot distribution of BCs
  ...
  scale_y_continuous("",expand = c(0,0),breaks = 1:65,labels = unique(coord.df$lab)[-2])+
  coord_fixed(ratio = 0.8)+ 
  labs(x = 'Depth (m)')
dev.off()

### Figure 2
nmds.sf.grid <- nmds.sf$grid
nmds.sf.grid.plot <- expand.grid(x=nmds.sf.grid$x,y=nmds.sf.grid$y)
nmds.sf.grid.plot$z <- as.vector(nmds.sf.grid$z)
nmds.sf.grid.plot <- data.frame(na.omit(nmds.sf.grid.plot))
ggplot() +
  stat_contour(data = nmds.sf.grid.plot, aes(x = x, y = y, z = z, color = ..level..),binwidth = 0.3)+
  geom_point(data = site.scores.fct, aes(x = NMDS1, y = NMDS2,fill = BC),size=4,pch=21) +
  geom_text(data = species.scores.fct, mapping = aes(x = NMDS1, y = NMDS2, label = species),alpha = 0.8, size = 5) +
  scale_fill_manual(values = col, name = "BC")+
...
### Figure 3
par(mfrow=c(3,2), mar = c(5.1,20, 4.1, 5))
barplot(main = 'Overall', sort(c.importance)[11:21],horiz = T,las=1, xlim = c(0,0.02),cex.names=0.9,cex.main=3,col='white', alpha = 0.5)
abline(v = imp.mean ,col="red4",lty=2)
barplot(main = 'BC1', sort(c.importance.BC1)[12:21],horiz = T,las=1, xlim = c(0,0.02),cex.names=0.9,cex.main=3,col='#FF66FF', alpha = 0.5)
abline(v = imp.mean.BC1 ,col="red4",lty=2)
barplot(main = 'BC2', sort(c.importance.BC2)[12:21],horiz = T,las=1, xlim = c(0,0.005),cex.names=0.9,cex.main=3,col='#99CCFF', alpha = 0.5)
abline(v = imp.mean.BC2 ,col="red4",lty=2)
barplot(main = 'BC3', sort(c.importance.BC3)[12:21],horiz = T,las=1, xlim = c(0,0.005),cex.names=0.9,cex.main=3,col='grey', alpha = 0.5)
abline(v = imp.mean.BC3 ,col="red4",lty=2)
barplot(main = 'BC4', sort(c.importance.BC4)[12:21],horiz = T,las=1, xlim = c(0,0.005),cex.names=0.9,cex.main=3,col='#B8E186', alpha = 0.5)
abline(v = imp.mean.BC4 ,col="red4",lty=2)
barplot(main = 'BC5', sort(c.importance.BC5)[12:21],horiz = T,las=1, xlim = c(0,0.02),cex.names=0.9,cex.main=3,col='#FFED6F', alpha = 0.5)
abline(v = imp.mean.BC5 ,col="red4",lty=2)
dev.off()

### Figure 4 - partial dependence plots
par(mfrow=c(5,6), mar = c(4,2, 1, 2))
...
plot(c.rf.BC1.Forest_land_use_log, type = 'l')
...
plot(c.rf.BC2.Tourist_visitors_log, type = 'l')
...
plot(c.rf.BC3.Wave_exposure, type = 'l')
...
plot(c.rf.BC4.Wave_exposure, type = 'l')
...
plot(c.rf.BC5.Mean_chl_a, type = 'l')
